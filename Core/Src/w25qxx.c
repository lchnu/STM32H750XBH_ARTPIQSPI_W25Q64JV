...
int W25QXX_ExitQPIMode(void)
{
    ...
    if ( HAL_OK == HAL_QSPI_Command(&hqspi, &cmd, 100) )
    {
...
void W25QXX_Read(uint8_t *pbuf, uint32_t addr, uint32_t size)
{
    QSPI_CommandTypeDef cmd;
...
void W25QXX_Write_NoCheck(uint8_t *pbuf, uint32_t addr, uint32_t size)
{
    uint32_t page_rem;
...
void W25QXX_Write(uint8_t *pbuf, uint32_t addr, uint32_t size)
{
    uint32_t sec_pos;
    uint32_t sec_off;
    uint32_t sec_rem;
...
void W25QXX_Block32K_Erase(uint32_t addr)
{
    QSPI_CommandTypeDef cmd;
...
void W25QXX_Block64K_Erase(uint32_t addr)
{
    QSPI_CommandTypeDef cmd;
